...
const planoService = new plano_service_1.PlanoService();
class PlanoController {
    /**
     * GET /plano/limites
     * Retorna os limites do plano do estabelecimento
     */
    async getLimites(req, res) {
        try {
    ...
    async getStatus(req, res) {
        try {
            ...
            // Calcula percentuais
            const percentualProdutos = limites.limiteProdutos > 0
                ? Math.round((uso.produtos / limites.limiteProdutos) * 100)
                : 0;
            const percentualUsuarios = limites.limiteUsuarios > 0
                ? Math.round((uso.usuarios / limites.limiteUsuarios) * 100)
                : 0;
            const percentualMovimentacao = limites.limiteMovimentacaoMensal > 0
                ? Math.round((uso.movimentacaoMes / limites.limiteMovimentacaoMensal) * 100)
                : 0;
            ...
            const status = {
                ...
                recomendacao: percentualProdutos >= 80 ||
